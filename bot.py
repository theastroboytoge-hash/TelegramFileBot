...
def add_alias_to_file(user_id, file_name, new_alias):
    ...
    if not row:
        ...
        return False
    aliases = row[0] or ""
    aliases = aliases + f",{new_alias}" if aliases else new_alias
...
def search_files_by_name(user_id, query):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT file_id, file_name FROM files WHERE user_id=? AND (file_name LIKE ? OR aliases LIKE ?)", 
...
async def receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        await update.message.reply_text("❌ لطفاً یک فایل معتبر (سند، عکس، ویدیو یا صدا) بفرستید.")
        return
...
async def delete_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("❌ لطفاً نام فایل را مشخص کنید: /delete <نام>")
        return
    file_name = " ".join(args)
    if delete_file_by_name(user.id, file_name):
        await update.message.reply_text(f"✅ فایل '{file_name}' حذف شد.")
    else:
        await update.message.reply_text(f"❌ فایلی با نام '{file_name}' یافت نشد.")
async def rename_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("❌ دستور: /rename <نام قدیم> <نام جدید>")
        return
    old_name = args[0]
    ...
    if update_file_name(user.id, old_name, new_name):
        ...
        await update.message.reply_text(f"❌ فایلی با نام '{old_name}' یافت نشد.")
async def add_alias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("❌ دستور: /addname <نام فایل> <نام مستعار>")
        return
    file_name = args[0]
    alias = " ".join(args[1:])
    if add_alias_to_file(user.id, file_name, alias):
        await update.message.reply_text(f"✅ نام مستعار '{alias}' به فایل '{file_name}' اضافه شد.")
    else:
        await update.message.reply_text(f"❌ فایلی با نام '{file_name}' یافت نشد.")
async def admin_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not files:
        ...
        return
    text = "📂 **همه فایل‌های کاربران:**\n"
    for uid, fid, fname in files:
